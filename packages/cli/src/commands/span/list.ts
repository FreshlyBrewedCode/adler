...
function printTree(spans: Span[], childrenMap: Map<string | null, Span[]>, parentId: string | null, depth: number, json: boolean): void {
  const children = childrenMap.get(parentId) ?? []
  for (const span of children) {
    const indent = "  ".repeat(depth)
    const prefix = depth === 0 ? "" : `${indent}└─ `
    const duration = formatDuration(span.started_at, span.finished_at)
    const parentInfo = span.parent_id ? ` (parent: ${span.parent_id.slice(0, 8)})` : ""
    if (json) {
      console.log(JSON.stringify(span))
    } else {
      console.log(`${prefix}${span.id.slice(0, 8)} [${span.status}] ${span.name} (${span.kind}) ${duration}${parentInfo}`)
    }
...
export const spanListCmd = new Command("list")
  ...
  .action(async (options: { json?: boolean }) => {
    ...
    try {
      const sessionId = resolveSessionId({ session: spanListCmd.optsWithGlobals().session })
      if (!sessionId) {
